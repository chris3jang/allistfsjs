...
const Data = () => {
	...
	const createItem = id => {
		callFetch('createItem', {id}).then(data => {
			...
			//determines the parent property for newly created item
			...
			const itemsAfterCreateOrdered = inOrder(itemsAfterCreate);
			setItems(itemsAfterCreateOrdered);
		})
	}

	const deleteItem = id => {
		callFetch('deleteItem', { id }).then(() => {
			const itemToDelete = items.find(item => item._id === id);
			const itemsBeforeDeleteByON = inOrder(items.slice());
	...
	const untabItem = id => {
		callFetch('untabItem', { id }).then(() => {
			...
			const getIncrementForItemToUntab = () => {
				...
				if(highestPrevSiblingDescendant) {
					return highestPrevSiblingDescendant.orderNumber - (itemToUntab.orderNumber + descendantItems.length);
				}
				return highestPrevSibling.orderNumber - (itemToUntab.orderNumber + descendantItems.length);
			}
			const incrementAmount = getIncrementForItemToUntab();
			const prevParent = items.find(item => itemToUntab.parent === item._id);
			const itemUntabbed = {
				...itemToUntab,
				orderNumber: itemToUntab.orderNumber + incrementAmount,
			...
			const newDescendantItems = descendantItems.map(item => {
				return {
					...item,
					orderNumber: item.orderNumber + incrementAmount,
					indentLevel: item.indentLevel - 1
				};
			});
			const siblingsToSkip = itemsByON.slice(itemToUntab.orderNumber + descendantItems.length + 1, itemToUntab.orderNumber + descendantItems.length + 1 + incrementAmount);
